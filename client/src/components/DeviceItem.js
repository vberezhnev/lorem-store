...
const DeviceItem = ({ device }) => {
  ...
  return (
    <Box
      className="flex flex-wrap m-2 w-[26vh] hover:scale-[1.01] shadow-md rounded-xl duration-200"
      onClick={() => navigate(DEVICE_ROUTE + '/' + device.id)}>
      <Box>
        <Box className="flex-none max-w-sm overflow-hidden rounded-t-xl">
          <img src={`http://localhost:7000/${device.img}`} className="h-auto w-full" alt="item" />
        </Box>

        <div className="p-3 flex-initial">
